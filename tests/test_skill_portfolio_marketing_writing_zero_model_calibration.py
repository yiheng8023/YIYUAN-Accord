...
from scripts.evaluate_skill_portfolio_marketing_writing_zero_model_calibration import (
    FIXTURE_PATH,
    PROTOCOL_PATH,
...
from scripts.validate_skill_portfolio_marketing_writing_zero_model_protocol import (
    validate_protocol,
)


class MarketingWritingZeroModelCalibrationTests(unittest.TestCase):
    @classmethod
    ...
    def test_heuristic_panel_cannot_be_promoted(self) -> None:
        protocol = copy.deepcopy(self.protocol)
        protocol["heuristicEvidenceBoundary"][
            "simulatedPanelTreatedAsIndependentExpertEvidence"
        ] = True

        with self.assertRaisesRegex(RuntimeError, "Heuristic evidence boundary drifted"):
    ...
    def test_claim_boundary_cannot_be_promoted(self) -> None:
        protocol = copy.deepcopy(self.protocol)
        protocol["claimBoundary"]["marketingEffectivenessProved"] = True

        with self.assertRaisesRegex(RuntimeError, "Claim boundary drifted"):
            self.evaluate(protocol=protocol)

    def test_repository_calibration_preserves_truth_and_preference_boundaries(self) -> None:
        ...
        self.assertEqual("effect.marketing-writing", report["effectGroupId"])
        ...
        self.assertTrue(report["accessCommsBoundaryReused"])
        self.assertFalse(report["heuristicPanelTreatedAsIndependentEvidence"])
