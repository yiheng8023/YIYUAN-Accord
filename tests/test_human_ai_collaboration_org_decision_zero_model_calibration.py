...
from scripts.validate_human_ai_collaboration_org_decision_zero_model_protocol import (
    validate_protocol,
)


class OrgDecisionZeroModelCalibrationTests(unittest.TestCase):
    @classmethod
    ...
    def test_repository_protocol_is_bounded_and_valid(self) -> None:
        ...
        self.assertEqual(15, report["caseCount"])
        self.assertEqual(14, report["faultCaseCount"])
    ...
    def test_low_power_affected_party_omission_is_not_hidden_by_power(self) -> None:
        report = self.evaluate()
        case = next(
            item
            for item in report["cases"]
            if item["faultClass"] == "low-power-affected-party-omission"
        )
        self.assertEqual(
            ["invariant-omitted:affected-parties"],
            case["activeLossIds"],
        )

    def test_decision_authority_transfer_is_rejected(self) -> None:
        report = self.evaluate()
        case = next(
            item
            for item in report["cases"]
            if item["faultClass"] == "decision-authority-transfer"
        )
        self.assertEqual(
            ["invariant-changed:decision-authority"],
            case["activeLossIds"],
        )
    ...
    def test_fault_label_cannot_hide_a_different_mutation(self) -> None:
        ...
        case = next(
            ...
            if item["faultClass"] == "dissent-omission"
        )
        case["overrides"] = {"dissentIds": []}
    ...
    def test_official_supplement_cannot_become_decision_authority(self) -> None:
        protocol = copy.deepcopy(self.protocol)
        protocol["conditionalOfficialSupplement"][
            "affectedPartyParticipationOrDecisionAuthoritySupplied"
        ] = True
        with self.assertRaisesRegex(RuntimeError, "Official supplement boundary drifted"):
            self.evaluate(protocol=protocol)
    ...
    def test_claim_boundary_promotion_fails_closed(self) -> None:
        fixture = copy.deepcopy(self.fixture)
        fixture["claimBoundary"]["affectedPartyParticipationProved"] = True
    ...
    def test_protocol_header_promotion_fails_closed(self) -> None:
        protocol = copy.deepcopy(self.protocol)
        protocol["status"] = "live-organization-verified"
