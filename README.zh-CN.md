...
- 一个尚未启用的 Codex 薄插件源码候选，其中包含任务级 Skill 与包内生命周期/捕获实验；
  它不执行任务仓库代码或 Git，但不是已接受的 v1.2 应用，并在可信运行时绑定前故意不可
  激活。当前受控符合性路线要求最终 O2 包只保留有证据的宿主残余增量，并证明精确激活、
  回滚、移除和清理；工作区发现入口目前只将源码候选标为 `AVAILABLE`；
