...
项目仍处于研究验证阶段；v0.2 当前为 `active`，正在推进一项已预注册的 Codex 与不同宿主
匹配任务，用来决定上下文载体的最小正确架构边界。该任务尚未调用模型，必须先取得注册中
限定的临时信任、费用与清理授权。仓库目前有机器可读的产品契约、确定性验证器和未启用的
参考适配器候选；O1 已由公开入口改写以及未安装的 Codex、
...
因而没有执行。所有任务的插件、信任、缓存、认证链接、进程和临时文件均已清理，所有
stopped 结果都不增加 O1-O5 进度。
...
不等于宿主兼容。本次 active 任务只使用官方文档所列支持子集，若精确宿主仍拒绝，任务
照样失败关闭。准确状态以仓库验证器为准。
