...
没有 active causal increment，O1-O5 仍全部为 false。仓库已有一个可直接应用、但明确
尚未验收的候选方法与最低质量 profile；未来合格任务必须在执行前绑定其精确 Git 和
内容身份，不能完成后倒推 O4 评分标准。该候选不增加 runtime，也不构成 O1-O5 结果。
当前机器状态以验证器输出为准。
