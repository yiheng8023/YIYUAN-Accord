...
**当前处于研究与验证阶段：**仓库现有内容是产品契约、验收模型、确定性验证器和一个
尚未启用的 Codex `SessionStart` 参考适配器候选；尚不包含 Agent 运行时、已经安装或
通过行为验收的适配器，也没有跨宿主证明。v0.2 当前为 `ready`，没有 active causal
increment，O1-O5 仍全部为 false。当前 program 已明确：如果主要目的是交付计划中的
实际产品结果，而不是为了演练或诊断 Harness，那么产品计划本身就是已授权的真实需求。
适配器只从当前仓库权威派生有限的接续上下文，尚未启用，也不证明运行行为或结果。当前
机器状态以验证器输出为准。
