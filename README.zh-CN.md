...
研究不是每个项目开场都必须执行的固定仪式。现有纯数据 `reconcile_closure` 接口
把“获取决策所需证据”当作普通的上下文责任：新鲜且充分的路线不增加研究；只有会
改变路线的物质未知才按当前权限与成本动态分配官方资料、源仓库、论文、宿主原生或
公共线索适配器。公共线索可以开启调查或反例，但不能单独支撑有后果的结论。便携内核
不因此新增研究专用 API、账号连接、常驻服务或具体宿主/模型绑定。
...
对于复杂项目，基线、计划、工序、验收和目标投影都是版本化的阶段投影，而不是固定真理。
阶段收官形成绑定精确表面、证据、有限声明与失效触发器的可引用节点快照。快照只声明稳定
locator 和 `containing-git-commit` 关系，不把自己的 SHA 写进对象；提交后以
`<containing-sha>:<self-locator>` 精确引用，因此既无自指，也不能自证后续门槛。
未来规划另作为非权威的演进视界，按需由项目全景、最新已接受快照和新鲜环境事实重新派生，
覆盖维护、迭代、更新、有限重构、宿主适配、退役或替换与后续开发；这些只是候选类别，
只有当前确有依据的一项通过权限、可行性、价值、生命周期与验收门槛后，才进入下一个
有界增量，不会冻结成自动执行的长期路线图。
...
直接 App Server 客户端只保留为 evaluator，不新增第二 API、浏览器桥、MCP、SDK 客户端或常驻服务。
...
更新后客户端当前入口、跨宿主价值或 release readiness。
