...
通过行为验收的适配器，也没有跨宿主证明。v0.2 当前有一个结果中立的 Codex 连续性
增量处于 active，O1-O5 仍全部为 false。该增量明确：如果主要目的是交付计划中的实际
产品结果，而不是为了演练或诊断 Harness，那么产品计划本身就是已授权的真实需求。
适配器只从当前仓库权威派生有限的接续上下文，尚未启用，也不证明运行行为或结果。
当前机器状态以验证器输出为准。
