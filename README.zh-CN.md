...
源原生原因、闭合源游标并验证目标激活，否则停止 cohort。测量执行前，不可变任务注册必须
绑定任务专属验证器的代码所有身份、已提交 locator、更早 revision 与 digest，且注册验证缝必须
实际通过其预测量检查；后续结果证据只能复用同一验证器身份。该验证器仍须证明完整来源游标
没有漏掉更早的合格需求、自然需求事件先于注册且独立测量事件晚于注册。只要仍
