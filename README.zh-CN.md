...
项目仍处于研究验证阶段；v0.2 的机器治理工序已经完成，仓库验证器报告
`completionState=accepted`：O1-O5 全部通过（`5/5`），G1-G4 全部通过（`4/4`）。
这表示有界研究版验收成立，不表示生产、公开发布、普适可移植或适配器已经长期安装生效。

O1 由公开入口、Codex Skill 源码和 Claude Skill 源码三个已接受结果支持。O2 使用实质不同
...
O5 使用同一个预注册自然任务，分别在 Codex CLI 0.147.0/gpt-5.6-sol 与不同宿主
