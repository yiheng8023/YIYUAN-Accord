...
Agent Autonomy Harness 是一个开放、Agent 中立、需求驱动的人机协作质量验证框架。它正在构建的
目标，是在真实任务中保持目标、能力路线、授权边界、生命周期、连续性、证据和清理的
一致性，而不是让用户亲自编排每个 Agent、Skill、MCP、Plugin、Hook、线程、工作树、
能力目录和管理器。

它不是“大而全的 Skills 列表”、固定能力目录或通用运行时。外围能力和发现渠道是可替换
输入；产品契约要求 Harness 先观察已有能力、识别真实缺口，仅在需要时发现候选，选择并
调度最小充分路线，并在需求结束时释放。

Harness 也不会为了“全栈自有”而重做已有充分实现的协议、注册表、网关、工具搜索、
运行时、治理内核或评价机制。每个实现决策都先核对来源绑定且注明时间的外部生态：
能复用或薄适配就不自研，确有集成缺口才组合，只有可重复的剩余语义缺口才新增实现。

长期产物是“需求到能力”的人机协作方法论、开放最低质量一致性剖面和可执行参考适配器。
剖面规定可检验行为与质量底线，不另造线协议、运行时、身份系统、审计日志或通用工具
模式。Codex 是第一条参考切片，不是可移植核心的依赖。

当前实现验证这份契约及其因果工序；任务执行、行为评价和跨宿主适配仍是 v0.2 的
planned 结果，不是当前运行时能力声明。

当前交付顺序采用 Codex-first：先以 Codex 作为参考宿主跑通第一条纵向切片，同时保持
产品语义 Agent 中立。参考路径稳定不等于通过跨宿主 O5；不同的第二 Agent 宿主或
运行时仍需单独证明。
...
对一个目标级已绑定任务，Harness 明确保持以下闭环：

1. 绑定真实目标、输入、授权和验证表面，不要求用户先指定能力路线；
2. 观察健康且已授权的现有能力，并评估缺口；
3. 仅在缺口确实需要时，通过来源绑定的自适应渠道发现候选；
4. 在有意义的副作用前选择并预览最小充分路线；
5. 只在已授予的任务边界内调度；
6. 观察、恢复并验证结果、用户干预和 claim ceiling；
7. 释放任务级暴露、清理，并留下可接续记录。
...
- O1：在预注册评价协议下，一个目标级自然真实任务以零用户能力编排干预完成
  “需求到能力”的自主闭环；
- O2：在重复目标级真实任务中降低用户能力编排干预；
- O3：通过自适应发现、build-versus-reuse 证据和生命周期决策，让广泛且持续变化的
  能力生态不再成为用户的认知负担；
- O4：形成已接受的 Agent 中立“需求到能力”协作方法论和开放最低质量一致性剖面；
  固定来源的外部参照组及充分的协议、运行时、身份、溯源与评价层必须先复用，软工只是
  第一参考剖面，不能靠自我命名为“世界级”；
...
对已绑定需求，先观察健康且已授权的现有能力。若不足，再依次优先合适的原生/运行时
或官方路线、已审查且维护良好的外部实现、现有能力组合；只有可重复的剩余缺口才允许
自研。

能力范围跟随需求而不是目录规模。需求结束后默认结束任务级暴露；保留为未激活候选
不同于持续激活，后者需要独立的价值证据和授权。
用户无需知道或指定能力、产品、发现渠道或调用语法；目录、供应商和发现渠道只是
自适应来源，不是产品权威。
