...
停止并完成清理；随后一个单独获批的 `gpt-5.6-luna`/low turn 在 `thread/start` 与 `thread/resume`
请求 `config` 中提供运行时 `bypass_hook_trust`。安装态同步 `SessionStart:resume` Hook 精确启动、完成各一次，
...
内核调用、产品结果、拓扑执行、窄化信任生命周期以及 Node 可移植性仍未证明。当前活动门槛仍是
GT-21：比较直接 App Server 事件路线与
首 turn Hook 路线，并在冻结字节的隔离证据臂闭合这条
最小实时链路；GT-20 随后验证最终组件集合在非空宿主中的事务化生命周期。全系统重验收、
精确 revision 观察和一次全新的独立审查仍受门控。修订后的
...
还不是“即插即用、即卸即走”的行为证明。形成未来候选之前，GT-21 必须先消费相关的官方
查询、动作、事件、能力、状态、拓扑、权限和生命周期面，再以冻结字节在一次性官方纯净与
候选启用隔离证据臂证明事件失效与重感知、纯内核调用、类型化最小上下文、Agent 可观察使用、
结果、独立后状态、充分简单路线静默和拓扑安全的新对话交接。安装、启用或 Hook 输出不能替代
这条链路。最终组件集合选定后，GT-20 才在
非空的隔离 Codex 与 Claude 配置域中证明：建立来源信任后，用户每次只需给出
一次生命周期意图，所有选中 Accord 组件由宿主编排；失败更新原子回滚；既有
配置、并发用户修改、共享与外来状态保持不变；移除后 Accord 注册、暴露、进程
和数据等活动状态为零。只有宿主明确所有、不可调用、按其文档自动限期回收且无需
用户介入的惰性缓存可以暂留；这不叫物理零残留，Accord 也不得绕开宿主生命周期
擅自删除。
