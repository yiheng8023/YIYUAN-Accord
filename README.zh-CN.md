...
3. 只有当前需求和授权确有必要时，才发现、安装、配置、启用、降级、停用、回滚、退役或
   持久化能力；
...
- 明确区分“可观测原生最小态”和“用户配置态”、且所有结果仍为 planned 的
  [v1.1 终态验收标准](product/acceptance.json)；
...
- 一个尚未启用的 Codex 薄插件源码候选；其中一个任务级 Skill 投影仅获 v0.2 Codex
  有界校准、尚未满足 v1.1 的方法；包内 Hook 只保存有界的压缩/清空计数并给出保守载体
  决策，不执行任务仓库代码或 Git，可在非 Harness 仓库任务中工作。它在绑定绝对可信
  运行时前故意不可激活，新的环境归因 v1.1 profile 绑定完成前也不得用于测量；工作区发现入口只将
  它标记为 `AVAILABLE`；
...
任何 v1.1 结果测量开始前，每份 O1-O5 注册都必须绑定验收权威中的环境归因契约。两类
...
执行中发生实质漂移则重新注册或诚实停止。Agent 执行所有宿主支持且已授权的操作；技术或
权威上只能由人完成的动作必须最小化、精确引导、事后验证，并在负担中与“把 Agent 工作甩
...
授权前若出现自然需求，cohort 必须停止，不能把它漏掉后继续。该协议只预先规定资格、隐私
安全的随机公开任务身份与固定的私域密钥来源消息绑定、分层、首个合格任务顺序纳入、最早前缀
停止规则、缺失数据处理、自然需求事件与测量事件的分离证明及声明上限，不预填未来任务
名单。后续每份任务注册必须身份唯一，并在 Git 历史上严格晚于这次冻结。Git 祖先关系
只能证明仓库顺序；来源转场必须保持同一密钥和 cohort 顺序，在目标面需求出现前证明
源原生原因、闭合源游标并验证目标激活，否则停止 cohort。任务专属验证器仍须证明完整
...
successor freeze 来源验证器。当前 v1.1 授权注册表保持为空，直到选择新的精确 profile、
来源与信任边界；这些验证器都不是结果验证器。
