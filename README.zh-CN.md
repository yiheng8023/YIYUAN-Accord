...
启用输入 Hook 后，开发包会把收到的原生输入文字保留在既有本机任务回执中，供恢复时按需读取。每份回执上限为 8 MiB，按任务生命周期处理；中断时即使尚未建立检查点，输入仍保留用于恢复，单纯退出不会将它删除。这份本地副本可能包含敏感文字。它不是完整对话或进度记录，也不产生新授权。保留与失败边界见[架构说明](docs/architecture.md#current-candidate-and-scoped-connections)。
