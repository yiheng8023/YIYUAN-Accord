...
>
> 可先看[能力边界](#已经证明什么还有什么没证明)、[版本变更](CHANGELOG.md)或[3.3 计划](docs/operations/PLAN-v3.3.md)（开发先聚焦 ChatGPT/Codex，功能尚未验收）。[3.2 历史计划](docs/operations/PLAN-v3.2.md)保留原范围。使用已接受的精确 tag，不从持续变化的分支安装。
...
- **可选检查点**：按 Agent 选择的条件核对任务文件，维护输入新鲜度和未完工作，并通过受支持的原生事件请求有界续做。任务语义、实际执行与恢复由 Agent 负责。
...
Accord 通过注入指引和可选提示影响 Agent 的判断与行动。被加载的内容会占用上下文；额外核验或工具调用还可能增加 Token、等待时间和费用。这些是需要衡量的运行成本，不能仅凭消耗增加就判断无益，也不能用少量成功样本承诺节省。应结合结果、可靠性、返工、用户负担和完整生命周期成本评估净收益，必要的安全与授权要求仍须满足。

可能的不利影响包括与其它指令冲突、挤占有效上下文、过度介入、不必要的询问或检查，以及错误路由和行为波动。开发观察已记录过未调用 Skill、预算内未交付、输出说明不准确，以及交接中未经授权发起来源任务归档的问题；相关指引已修订，但这不证明新包在所有宿主中已消除此类问题，也不能把每次失败都归因于 Accord。范围和反例见[开发源](product/development.json)；普通使用中的增益仍待验证。

Accord 组织和引导已有能力的使用，**不训练或改写模型、不扩大模型原生上下文容量，也不突破宿主的接口、权限和执行边界**。它不能保证任务完成、自动恢复或可靠交接。获授权接入外部工具可以扩展组合系统能处理的任务范围，同时增加依赖和成本；这不等于提高模型或宿主本体的能力上限。

可检验的关键约束应使用可重复检查，并用反例检验检查本身。检查也可能遗漏需求或固化错误预期；意图、授权、判定依据和实际结果仍需核对。当前插件提供指引和上下文提示，不拦截宿主工具调用，不能作为权限屏障；仓库检查也不能证明宿主已执行这些约束。维护方法见[贡献指南](CONTRIBUTING.md#verification-and-publication)。

在已验证的任务中比较是否值得保留介入。若成本或干扰超过收益，应按宿主支持的方式缩小使用范围、停用或移除，并核对剩余状态。

## 3.2 用什么标准改进

安全和约定的结果，先于减少代码、成本或介入。满足这些要求的前提下，路线应随任务调整，而不是套用全局固定 SOP。

用户意图与授权、可调整的计划、进度展示分别承担职责。新证据可以修订路线、工序和有理由变更的验收，进度视图映射当前计划；任何计划或调度工具都不成为项目的强制执行前提。
...
让 Agent 分别核对精确来源与已安装字节、启用登记、当前使用会话的 Skill 可见性及相关调用。安装或更新后，已经打开的会话可能仍保留旧的技能目录；另一个新会话加载成功，不代表原会话已刷新。优先使用宿主支持的刷新或重新进入方式，并核对原任务的实际暴露。独立 CLI／App Server 查询不能证明已经运行的 Desktop 进程已刷新；若新任务也仍暴露旧目录，应按宿主支持的方式重启应用，保留未完成工作，并在重启后核对实际加载版本。[官方技能刷新说明](https://learn.chatgpt.com/docs/build-skills)。确需新建任务时先获得相应授权、保留目标与未完成工作，不自动新建或归档任务。列表或重载命令本身不够。

如果安装后“没有作用”，先区分技能未暴露、已暴露但未选用、已选用但执行失败，以及执行后没有可观察收益。Hook 提示不等于完整 Skill 已加载；检查点的 `unbound` 只表示没有绑定文件结果条件，不能单独证明插件失效。Agent 应完成这些检查和受支持的恢复，不要求用户反复点名 Skill 来维持普通使用。

验证输入触发时，区分用户在输入框提交的消息与工具输出形式的任务续做；后者不自动等同 `UserPromptSubmit`。分别核对实际收到的事件、Skill 读取和结果，避免用一种入口代验另一种入口。
