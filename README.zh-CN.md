...
项目仍处于研究验证阶段，当前有且仅有一个已预注册的 Codex 任务级 Skill 投影增量。
仓库目前有机器可读的产品契约、确定性验证器和未启用的参考适配器候选；O1 已由一个
任务绑定且经用户认可的公开入口结果验证，O2-O5 仍未通过
（结果 `1/5`），也还没有通过验收的适配器运行能力或跨宿主证明。准确状态以仓库验证器
为准。
