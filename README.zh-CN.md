...
持续移动的 `main` 当前包含尚未发布的 v3.1.0 候选：它把先前的能力差集概括
修正为上下文自适应的协作闭环，并增加一个无状态 `SessionStart` 激活提醒。
这只是开发证据，不是安装源或已发布升级。
...
Hook、MCP server、App、状态存储、后台进程或自动项目修改。尚未发布的
v3.1.0 候选还包含一个无状态 `SessionStart` 上下文 Hook：它不读取会话原文、
不写状态、不启动后台进程。这些只是版本化包事实，不是产品的永久禁区；
