...
五个有界 outcome-neutral 切片已在未来测量之前提交 O4 专属验证器，把公开安全投影绑定到
Codex 原始宿主消息和仓库观察，并使受控载体路线在注册后可达。一个精确公开目标必须在原生
压缩与不继承历史的新同目标线程中保持 active；只有目的线程恢复目标和仓库权威后，才可清除
来源 goal 并归档来源载体。全历史分叉只保留为可选拓扑分支，不作为上下文减载。五次切片都
退出当前图，不执行 O4 测量，也不获得结果信用。
