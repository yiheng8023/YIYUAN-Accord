...
开发包还提供只读 MCP 工具，用于检查根任务已存状态，以及在恢复时取回已捕获输入。它们复用现有检查点和同一个宿主管理的 Node 标准输入输出进程，无需新增账户或远端服务。捕获输入不是完整历史或新的授权。宿主调用身份、调用者选择的工作目录与权限、当前意图分别核对；子任务不会自动承接共享会话的状态。这些工具不重放输入、不解除暂停，也不派发交接。
