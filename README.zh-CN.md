...
有界宿主证据只覆盖一条事件到结果路线、一条保持静默的充分路线、一次已验证的新对话交接，以及一次性非空 Windows Codex 与 Claude 重放中的历史生命周期子事实。精确审查切点 `c5a0668` 否定了宽泛 GT-20 结论；精确 `7a3950e` 已在修复权威绑定、暂存摘要可重算性和跨平台路径隐私后验证纠正后的有界 schema-v6 重放。受影响面已经重新验收，四项新鲜独立精确树审查正在进行。这不证明自动回滚、当前客户端行为、跨系统等价、产品价值、仓库候选或发布就绪。
