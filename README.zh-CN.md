...
已预注册一条自然任务，验证 Codex 原生上下文压缩后，Agent 能否在同一任务内保持用户
目标和仓库连续性，而不把恢复或 Codex 编排交还用户。只有后续真实行为、任务底线、
清理和命名人类判断全部形成证据，O1 才可能通过。当前机器状态以验证器输出为准。

首次注册后压缩只计为结果中性的校准：连续性行为没有丢失，但预注册证据接缝写错了
Codex 输出记录类型。当前注册已明确排除该次校准；修正接缝提交并推送后发生的下一次
压缩才有资格进入正式测量，该次校准对 O1-O5 计零。
