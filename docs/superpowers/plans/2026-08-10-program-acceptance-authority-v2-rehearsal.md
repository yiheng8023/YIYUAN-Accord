...
- The migration inventory stores symbolic `patternId` values and symbolic binding-policy tokens. Matched content values live only in the discovery module's constant map. Under the line-708 exception, one decoded occurrence host `path` may equal the legacy-path literal; its deterministic physical JSON wire form escapes the slash as `\/` so raw-byte discovery does not recursively inventory the inventory itself. The raw legacy ID and all governance/action/non-path fields have no exception.
