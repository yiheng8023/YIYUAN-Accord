...
用户随后追问：Skill目前相当于Accord的抽象，若不作为上游是否会遗漏整体职责。当前代码确有Skill与Hook核心指导两处语义承载；“不保证每轮选中Skill”不等于允许整体规范缺省。Agent建议统一维护共同语义，由Hook/宿主承担可靠送达、状态保护和反馈，必要细节在依赖动作前取得；这是待核对重复、差异及可行实现的设计建议，尚未据此批准或实施整套载体重构，用户目标与授权仍高于包内指导。
