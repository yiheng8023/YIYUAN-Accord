...
执行连接采用随包Node适配器，复用宿主已持有的原生控制连接、作用域持久记录与独立核验，不新增常驻服务或私有观察体系。控制者条件下的机械派发、多轮接管和失败分支，以及能力缺口下的一次真实模型自主源提议/语义接管已有有界证据，见接续；下一补上下文压力择时、无益调用反例、实际入口接入及失败恢复，不重做已闭合的小链。普通Desktop/IDE未提供该连接时保持未知，不搜索私有IPC或另造用户入口绕过边界。工序仍服从实际依赖，原生压缩和先前普通任务/受控交接结果按精确范围复用。
