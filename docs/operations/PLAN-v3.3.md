...
节点：N33-20260909 · 修订：r25 · 2026-09-15。状态：落实已确认的可靠入口、Skill分层与有界自举共识，继续现成机制复用和3.3必要交付；功能开发与必要验收未完成；用户已要求完整验收后发布3.3。
...
当前先保留一个协调Skill，数量随实际职责和证据调整；以整体协作要求及必要底线作入口，按能力协作、恢复/交接、验证/包生命周期的真实情境分层提供细节。指针条件应能在未读正文时识别，并要求在依赖动作前取得必要细节；避免先知道缺失规则才能发现自己缺规则的循环。简单且已由有效入口职责覆盖的工作不机械加载全部资料；确有独立调用价值才增加Skill。引用失效或必要指导不可得时保留已知底线和未完责任，解决依赖或明确限制，不伪称完整覆盖。
...
1. 对齐现有17项必要范围及九个已定义案例，明确当前真实缺口、可复用机制、适用入口和验收证据；优先修正不贴近所声称用途的测试条件，而不增加一套台账。既有局部事实按精确依赖复用，历史失败不改写。
...
同版本原地编辑仍可能命中旧缓存；开发更新用受支持的新构建身份及安装流程，并回读实际字节。[Hook信任](https://learn.chatgpt.com/docs/hooks)按声明身份和有效策略判断，声明改变可能需重新信任；声明Hash不是脚本内容Hash，也不能据Hash未变推断包未变。当前生命周期观察已有精确安装、坏候选拒绝、同一健康包重试和重启/卸载后的状态保留；它没有执行有效新版本的热切换、真实宿主跨版本升级或全入口失效后由外部恢复者接管。A06按下列对应判据补受影响证据；现有固定CLI版本CI不代替宿主升级验证，适配器不锁定版本也不表示所有版本已兼容。支持条件漂移或入口整体不可用时，复用预先保留的宿主控制/其它获准恢复者，不能让已失效Hook成为唯一恢复者。
