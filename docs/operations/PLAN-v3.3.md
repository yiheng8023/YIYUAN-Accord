...
1. **主线先闭合可靠上游与普通任务交付。** 启动/普通输入的核心职责注入、按需Skill分层、transcript感知接线及恢复提示已经实现，不再列作尚待开发。优先在已具备条件的真实入口，绑定当前精确包的实际加载、授权与必要运行条件，用代表性普通任务检验“上游职责生效→必要细节和能力被采用→执行反馈与纠偏→交付核验→必要接续”。共享安装、源Hook投影和已安装候选各有身份，不能互相代验。若必要职责未进入执行或恢复后丢失，先修该上游断点；若指导已进入但行为不符，修判断/反馈或对应实现，不能只反复增加说明。Skill是可选载体之一，不把每轮加载全量Skill当成完成条件，也不以注入成功结束此主线。 9月24日对既有完整安装五轮原件与正式定义的核对确认：历史正确结果保留，但执行条件不同、缺正式观察/审查连接和原观察时效，不能事后套入旧case。已补现有准入器的安全原因码诊断，未更改86400秒规则或其它判据。下一次有必要的普通行为执行前，同步绑定真实入口/条件、不可变候选与依赖和正式观察/复查通道；不先跑开发观察，再为通过改定义或改时间。 已在原CLI观察器接入可选--admission-case准备/逐轮复核，复用正式定义和Git包身份；当前仅核对可观察的准备参数，不产出事实、审查或验收通过。新场景须事先声明结构化执行条件，原缺此字段的案例不补写历史；实际行为与独立observe/recheck仍待下一必要任务。 同日修复观察器在后续复核/检查异常时丢失汇总的断点：保留原生回执、已知任务和部分结果，未知效果不重放；后续文件pass不追改旧执行。剩余主线主要是必要普通任务的正式调用者编排，不再为同一连接另建通用框架。 已确认准入核心已有observe/recheck，旧发布02拒绝来自真实失败，不新增另一套验收框架。修复现有入口观察器的保留副本同名覆盖，增加显式只读retained回看，供工作区清理后的独立源核对；旧执行、时间和判据不改标，记录/字节一致不等于业务通过。 当前开发容量额度及对应测量理由集中在[机器投影](../../product/development.json)的changeBoundary.complexityBudget，保持同一5%预留和独立的指导体量边界。该工程分配不改变业务验收、原失败或证据时效；历史额度保留在Git中，不在活动计划重复维护易漂移的数值。
   9月26日当前完整安装的一次五阶段普通协调执行已闭合：预绑60219fff，目标/授权更正与暂停恢复两个case经既有observe/recheck正式准入。原输入、判据、限额及历史身份保持；逐轮默认模式/Goal时点与受保护文件已核对。详见[本次结果和剩余范围](CONTINUATION.md)。这两项不再作为下一次重复执行任务；主线余项仍是必要连续性、其它适用入口和整体影响，不能将两scope通过扩大为完整A项或自动交接。
