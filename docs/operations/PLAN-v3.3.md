...
当前普通交付工作段已具体选择W08所需的双语用户说明草案及结构字段/清单；沿A04原systemic-correction scope前瞻绑定[02案](../../product/cases/release-copy-v3.3.json)。两条维护者预绑的普通输入检验从旧版用户取向到非技术首次使用者的全局修订，并产出可复用发布准备材料；四份成品的事实、受众和未完状态都要核对，原件保持，阶段失败不由评估者救场后称自主通过。当前CLI0.156.1、完整安装01947、Luna/medium和边界已写入机器投影，先冻结再执行；旧01及其它历史观察不追改、不重复跑来填数。新案已按01397fbe执行但未通过：首轮有源证据及职责表述错误，次轮被预绑输出上限中止、清单未更新。失败/原额度保持，不以盲目重跑或下调判据改判；主代理单独校订后的双语/字段/清单已纳入发布准备，不能代验原模型路线自主成功。执行前复核纠正机器投影的过早完成风险：A04同时要求原有system-integration范围承接更广历史与恢复未决，02单独通过不能关闭整项；不增加必要范围或另开试验。必要scope仍17，新增一个冻结数据文件使文件分配171→172；没有新增产品控制器、模式或第三方模型服务。
