...
同日将下一必要普通工作绑定为[工序复盘](../../product/cases/workflow-retro-v3.3.json)：root按用户既有委托选用现装explicit-only retro，worker原生显式调用并交付有依据的改进建议或无需改动判断，独立评估者核对原件与后态。保留第三方源/策略，明确宿主指导与业务事实边界，前瞻绑定effective-user-environment子范围；不将其改写为worker隐式匹配或自主择时，不重跑已结束旧案，也不自动实施复盘建议。
