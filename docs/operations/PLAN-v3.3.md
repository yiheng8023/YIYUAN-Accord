...
节点：N33-20260909 · 修订：r31 · 2026-09-23。状态：3.3内化适用外部设计并借力宿主完成职责，专用第三方决策模型集成后置；功能开发与必要验收未完成，完整验收后发布3.3的条件授权保持。
...
2026-09-20校准CI节奏：本地提交与远端推送分开，小改先做受影响检查，相关改动形成完整工作段后再集中推送；不按对话轮次触发全矩阵。完整跨平台验证按集成节点、相关平台变化和最终发布候选安排，必要覆盖及发布门槛保持。正在执行的有效CI不因低价值推送被反复取消，独立本地开发仍可继续；真正改变检查前提的修复按实际需要推送，不设固定等待时长。纯接续/流程记录按相关静态检查处理，不把这一点扩大为跳过Skill、Hook、运行时或候选包变化的必要验证。先改执行节奏，不为此新增调度系统或默认使用skip ci。用户不必理解CI或等待后台工作结束后才交流；普通消息按当前任务解释，不自动取消在途工作。实际暂停、取消或需求变化按意图、影响和权限处理，恢复时先核对服务端真实状态，交付判定仍绑定所需版本及结果。

2026-09-25将上述CI责任落实到原工作流：push与手动验证按不可变提交SHA分组，不因后续提交取消正在执行的另一候选；PR检查仍按PR引用分组，允许新候选替换旧候选。同一SHA重复触发按宿主默认只保留一个等待项，不承诺全部排队。普通聊天不触发推送或取消；必要修正与其它工作可继续，Agent保留运行ID、目标SHA和未完核验责任，并以实际服务端结果判定。未新增调度器，矩阵、测试和发布底线保持。
...
模型能力、账号资格、当前派发接口支持、客户端显示/启用开关与本次有效选择分开核对。2026-09-23用户说明并展示本机手动开放Max/Ultra；这只证明该环境的自定义可见项，不证明默认用户可见、任一入口可调用或已选择该强度。不得为了扩充候选而擅自打开隐藏/选择加入的档位，也不得把API、界面滑块或父代理选项视为子代理接口的等价能力。新模型或版本变化触发有界重评；质量底线、实际结果及复核/返工/缓存等总成本决定分工，不固化强主模型加轻子模型的梯子，主代理把关不替代结果证据。
...
第三方Skill协调并入W02/W06/W08和A02/A06/A08：由Agent在用户委托范围内判断何时选用何种能力，非技术用户无需记住名称或调用时机。复用宿主及原管理器的盘点，区分安装、启用、可发现、原生隐式匹配、主动选定和实际执行。作者默认关闭隐式调用不等于用户永久不能授权协调；保留原源文件与管理权，在真实委托涵盖选择、宿主支持显式调用且目标任务控制权成立时，可以由协调层代选并通过该原生路径加载。记录原需求、委托范围及协调者选择，不冒充用户亲自点名，也不把构造输入当授权证明。停用、明确排除及新的权限/数据/费用边界继续有效；已有明确委托不要求每次重新确认。没有可用路径时选择合适替代或说明具体缺口，不偷偷读取/复制以规避有效限制，不先建通用路由器或批量改第三方源文件。调用前判断流程的适用性与介入程度，之后核验实际结果、暂停和退出后态。

2026-09-23核对[官方显式调用接口](https://learn.chatgpt.com/docs/app-server#start-a-turn-invoke-a-skill)与固定[0.156.0选择源码](https://github.com/openai/codex/blob/fe74a774532af67b5a4a3dec03ce9469e17f89af/codex-rs/skills/src/selection.rs)：已启用且关闭隐式匹配的Skill仍有原生显式加载路径；停用项被选择器拒绝。原生逐Skill配置仅支持名称/路径和enabled，没有隐式策略覆盖字段。隔离固定响应观察证实普通输入未载入测试Skill正文、显式输入载入且原文件保持；不证明协调者的语义选择、委托来源、当前Desktop/CLI自注入接口或普通用户全链。SDK可控入口、当前任务可用接口与其它入口分别核对，不因官方接口存在便承诺全端自动调用。

落实顺序：并入正在收尾的交接验证工作段及后续既有工序；相关缺口先纳入能力协调指导和既有动态路由/生命周期案例的判据，在合适的普通任务中复用验证，不为每个Skill或每种档位另建一轮实验。静态盘点与此次文档对齐不提升完成计数。
...
2026-09-25完成两项结束实例的当前适用性处置：失败的发布准备02与普通CLI代码审查01从活动cases转入developmentObservations中的历史引用，完整旧定义以d6c27f6b的Git对象定位，原执行身份、预算、原件和失败保持。原普通CLI01暂留，默认环境、恢复及其当前适用性仍须判断。所有17项必要scope及F/A判据、职责、质量、场景和条件不变；没有新实例时保留未完覆盖，空案例范围及缺场景不能通过。先有新正例才能停止把旧执行列为活动实例不是既定前提；需要的是下一真实任务在执行前完成适用条件绑定，不是为了填补清单制造工作或把全部历史失败永久叠加为发布门槛。
...
当前执行状态：用户已恢复推进，并于2026-09-22明确3.3以设计内化和宿主协作为主线，Jev/Laya等专用决策模型产品接入留待后续版本评估。取消将此类候选的追加试用或旁路接入作为本版下一动作；沿现有必要功能与验收推进，不另建自动唤醒。
...
2026-09-25用户再次要求继续并加快进度，明确自动压缩只是线程过长。停止追加该反馈的原因调查或确认工序，保持原生压缩与当前目标连续；普通交付主线的实现、独立审查和必要托管检查按真实依赖并行推进，不因等待CI闲置可独立完成的工作。
...
本批继续复用原生持久历史，已接通已settle目标的新controller恢复；Windows单次真实固定响应、原件保护和独立回读通过，Linux/macOS同批原生CI及artifact独立回读也通过；整体CI的离线测试目录别名错误另行修正，不改原生通过范围或放松判据。正常释放后恢复与未确认效果/异常中断分开，后者先按既有持久回执查后态，不清active transfer或自动重放。已核协议与两轮交接不再重复实验；后续工作针对真实择时、普通入口接入与必要失败恢复的剩余依赖。 9月24日回查补齐SDK源创建的持久性确认：请求和实际回执均须明确ephemeral=false，未经确认不绑定writer或发送业务轮次，保留已创建ID与原回执供对账。此为已有可恢复源契约的前置检查，不新增状态层或通用恢复框架；回执完全丢失仍保留原限制，115项本地检查不代验完整A05或普通模型择时。a6505491的CI11/11通过，开发安装已同步01947，当前公开0.156.1 Schema也未提供按创建请求ID认领源任务的接口；无精确身份时保留未知和owner对账，不把正常的不重放边界扩成必须推测恢复的新工程。
...
1. **主线先闭合可靠上游与普通任务交付。** 启动/普通输入的核心职责注入、按需Skill分层、transcript感知接线及恢复提示已经实现，不再列作尚待开发。优先在已具备条件的真实入口，绑定当前精确包的实际加载、授权与必要运行条件，用代表性普通任务检验“上游职责生效→必要细节和能力被采用→执行反馈与纠偏→交付核验→必要接续”。共享安装、源Hook投影和已安装候选各有身份，不能互相代验。若必要职责未进入执行或恢复后丢失，先修该上游断点；若指导已进入但行为不符，修判断/反馈或对应实现，不能只反复增加说明。Skill是可选载体之一，不把每轮加载全量Skill当成完成条件，也不以注入成功结束此主线。 9月24日对既有完整安装五轮原件与正式定义的核对确认：历史正确结果保留，但执行条件不同、缺正式观察/审查连接和原观察时效，不能事后套入旧case。已补现有准入器的安全原因码诊断，未更改86400秒规则或其它判据。下一次有必要的普通行为执行前，同步绑定真实入口/条件、不可变候选与依赖和正式观察/复查通道；不先跑开发观察，再为通过改定义或改时间。 已在原CLI观察器接入可选--admission-case准备/逐轮复核，复用正式定义和Git包身份；当前仅核对可观察的准备参数，不产出事实、审查或验收通过。新场景须事先声明结构化执行条件，原缺此字段的案例不补写历史；实际行为与独立observe/recheck仍待下一必要任务。 同日修复观察器在后续复核/检查异常时丢失汇总的断点：保留原生回执、已知任务和部分结果，未知效果不重放；后续文件pass不追改旧执行。剩余主线主要是必要普通任务的正式调用者编排，不再为同一连接另建通用框架。 已确认准入核心已有observe/recheck，旧发布02拒绝来自真实失败，不新增另一套验收框架。修复现有入口观察器的保留副本同名覆盖，增加显式只读retained回看，供工作区清理后的独立源核对；旧执行、时间和判据不改标，记录/字节一致不等于业务通过。 当前开发容量额度及对应测量理由集中在[机器投影](../../product/development.json)的changeBoundary.complexityBudget，保持同一5%预留和独立的指导体量边界。该工程分配不改变业务验收、原失败或证据时效；历史额度保留在Git中，不在活动计划重复维护易漂移的数值。
   9月26日当前完整安装的一次五阶段普通协调执行已闭合：预绑60219fff，目标/授权更正与暂停恢复两个case经既有observe/recheck正式准入。原输入、判据、限额及历史身份保持；逐轮默认模式/Goal时点与受保护文件已核对。9月27日另以一次前瞻绑定的Windows基础CI闭合029a1c58的SDK生命周期/作用域暴露两case，复用原runner和准入接口，未新增runtime。详见[本次结果和剩余范围](CONTINUATION.md)。完成实例不作为下一次重复执行任务；各自候选和条件不能机械叠加，主线余项仍是必要连续性、其它适用入口和整体影响，不能从子scope通过推成完整A项或自动交接。
...
9月25日回查确认SDK机械链已有实现，但正常提议尚缺“不必交接、留在源继续”的反馈出口；已在原source session补齐，保留严格提议身份及写者核对，暂停、撤权、未知或发送失败不能按正常拒绝继续。此变更为W05已有自主择时职责提供必要反馈路径，不增加宿主控制器或新验收范围。后续仍须由实际持有连接的调用者，在必要真实工作中核验判断、接管和结果，不能从这项回归推成普通Desktop/CLI已自动采用。

同日已有必要的预检诊断修复通过现装202615 SDK源会话实际执行：任务内调用者持有原生连接，协调者代选implement，模型查询上下文并交付两文件补丁；主线程纠正测试后集成。源轮次完成，但私有调用者等待主线程答复超时，失败和进程退出分别保留，不称完整调用者成功。未提出交接，不能据此增加自主转移验收；已证明的真实源执行和Skill协调不再反复探测，继续处理剩余业务接续及入口差异。详见[原始范围与收尾](PROCEDURE-v3.3.md#真实sdk源开发与预检诊断修复2026-09-25)。

同日的[必要工序复盘](../../product/cases/workflow-retro-v3.3.json)已执行：root按用户委托代选现装explicit-only retro，CLI原生加载正文并由worker调用依赖、产出有源报告，第三方源/策略保持。它证明这一协调调用路径实际可用，不证明worker隐式匹配、自主选型、所有Skill或自动交接。独立评审确认事实及排序，但下一工作段仍未指明具体任务/成果；正式准入保留consequence-mismatch，不改报告或降低判据。该结束实例转入带e74c4a4f定义/执行引用的历史，当前有效用户环境贡献仍待合格新证据；其它10活动案、17范围及原有结果要求不变，不重跑旧案填数。现有基线/工序没有因本次复盘被整体重写，建议按实际源据和既有授权处理。

9月23日普通派生报告任务已核实34028入口参与，但源记录对照不足，首次成品需要评估者纠正；不升级为自主通过。源码74845在原职责内改写短入口核验段和结果审查路由，保持证据的对象、时点与范围，当前可见性不足不自动推翻原条件下有效结果。运行时和Stop判据不变；只通过静态/接线检查，行为改善另待必要实际任务。沿W01/W03/W04/W07与既有A03/A04/A07推进，不新增模型服务、固定评审SOP或必要scope，不重复原批处理或报告试验取代剩余交付。

9月23日云端一次受控接入已得到真实差异：本地注册/安装和精确包核对成立，但可用0.144.0-alpha.4原生目录只发现两个SessionStart、UserPromptSubmit、Stop，缺少SessionEnd和Interrupt。本次按预绑6项授信门槛停止并回滚，原任务采用没有执行。后续按事件对应的职责审查兼容路径：必需上游、暂停/未知效果保护、恢复及状态回收分别找到充分执行者；可复用宿主取消、恢复与现有显式状态退休，但各自仍需实际证据。不能把本机声明数量设成通用前提，也不能直接降低本次门槛追认通过；不因此新增通用运行框架或放弃云端。

上述职责审查已完成源码层判断：精确0.144.0-alpha.4取消当前任务并记录TurnAborted，取消错误不进入正常Stop分支；缺少Interrupt不等于不能停止。Accord旧回执不自行启动执行，后续原生输入/resume重新约束旧状态；外部已发生效果和写者仍需核对。缺少SessionEnd时复用已有显式状态退休，无存活责任者就保持回收未知。7项既有本地定向检查通过，不代验云端；独立审查据实际取消源码修正了迟到Stop风险假设。目前没有据此新增运行时的理由。9月24日用户确认后，四Hook新方案已执行并回滚：74845注册/24文件安装核对及四项原生授信成立，同容器下一输入仍未获得Accord指导或状态工具，故原任务采用未成立。相邻两轮hostname与授信后配置摘要一致，不能把本次问题归给容器丢失；安装进程的发现不刷新原控制者。后续源码复核确认0.144逐轮构造仍来自已存会话配置，插件管理器复用缓存；安装和reload_user_config只刷新接收请求的server及其线程。现有独立检查连接不能当原控制者连接，当前未取得受支持的原任务刷新入口。官方setup/maintenance可作为启动前准备候选，不能据此保证加载。9月24日官方编辑入口曾返回403；9月25日只读复查经官方编辑按钮已正常打开，仓库目录、自动初始化、Agent网络关闭及缓存开启保持，未保存或启动任务。该界面阻塞已解除，不再等待用户重复核对；原控制者装载仍未知；随后获准的单次启动前方案在环境设置阶段32秒后失败，尚未进入Agent，已恢复自动setup，原案不重试。恢复后的独立官方终端观察到启动期默认配置尚不存在、CLI不在PATH；先核启动期与运行期的配置归属，不能沿用运行中任务前提或把离线守卫测试当真实装载通过。原失败容器细部后态仍未知；后续仅在私有repair稿修正显式目标配置绑定、首次无config.toml的预检与失败阶段/条件/可能写入的诊断，17项离线检查通过，未生成新setup或重开云端。原案保留as-run，目标Agent配置归属仍待独立依据，不编写推测性安装器、不循环重装或重复旧协议试验。结果及恢复边界在私有accord-cloud-route-20260923-01/cloud-adoption-result.json；旧六项试验不改标。基线与A01–A08结果判据已核对，无需改变17项范围或r31共识。 9月25日另获准的一次只读云端诊断已结束：运行期可见CODEX_HOME=/opt/codex，后续另一个App Server进程具有-c覆盖参数；仍未证明实际模型控制者的配置加载或刷新路径。一个任务URL内三条输入、两轮命令及界面计数9→12均按实保留；没有安装、授信、设置修改或旧案重放。详见[有界诊断及限制](PROCEDURE-v3.3.md#只读云端配置来源诊断2026-09-25)。当前停止基于路径猜测追加探针；云端缺口不阻止条件充分的普通交付主线，出现能改变判断的直接机制依据再推进其依赖动作。

2026-09-24普通交付工作段选择了W08所需的双语用户说明草案及结构字段/清单；沿A04原systemic-correction scope前瞻绑定[02案](../../product/cases/release-copy-v3.3.json)。两条维护者预绑的普通输入检验从旧版用户取向到非技术首次使用者的全局修订，并产出可复用发布准备材料；四份成品的事实、受众和未完状态都要核对，原件保持，阶段失败不由评估者救场后称自主通过。当时CLI0.156.1、完整安装01947、Luna/medium和边界已写入机器投影，先冻结再执行；旧01及其它历史观察不追改、不重复跑来填数。新案已按01397fbe执行但未通过：首轮有源证据及职责表述错误，次轮被预绑输出上限中止、清单未更新。失败/原额度保持，不以盲目重跑或下调判据改判；主代理单独校订后的双语/字段/清单已纳入发布准备，不能代验原模型路线自主成功。 后续普通任务须在执行前明确被验主代理、受委派worker、内部核验和独立评估者的边界；允许已授权的动态组合，不把便宜worker必须独立承担全部核验设成通用要求，也不把事后外部救场追认为原路线自主成功。选路按具体后果、实际表现和总成本校准，不由单例固定模型梯子。执行前复核纠正机器投影的过早完成风险：A04同时要求原有system-integration范围承接更广历史与恢复未决，02单独通过不能关闭整项；不增加必要范围或另开试验。必要scope仍17，新增一个冻结数据文件使文件分配171→172；没有新增产品控制器、模式或第三方模型服务。

9月25日独立推进W05已有恢复职责：普通SDK源在创建/绑定已确认、首次交接前丢失控制者时，原API无法恢复执行器。复用当前recorder claim与原生resume扩展同一恢复入口，保留完整来源、暂停、效果、旧控制者静止、单写者及后验工具/历史检查；明确排除首次ACK未知、活跃交接与旧token重放。25项离线会话回归及独立审查通过，无新增服务或schema；公共原生resume链继续回归，新分支实际行为不借旧证据代验。此独立缺口不替代普通任务交付/系统整合主线，不因此重跑已失败02案。
...
ac5dd3e8的Linux/macOS同一原生组合已独立回读通过。已完成的恢复切片复用原生持久会话和现有SQLite标记，覆盖已settle且前次执行已静止的controller更换：先独立核对当前权限/暂停和效果，再以已确认回执CAS claim，随后resume并核对原生后态；旧依据不能重复消费。未知效果和活跃transfer继续保留，不新增数据库schema或常驻服务。三平台原生验证使用原已结束场景的只读副本，旧证据未被恢复流程改写；73f4c6b8完整CI已通过。固定响应只回答恢复接线，未知在途效果后的完整接续、模型择时与业务恢复仍分别推进。源码回查发现reconcile成功尚无调用路径进入释放/settle/restore；本批补此确定性断边，并覆盖收尾自身的确认丢失及已知controller替换，93项本地组合通过。下一仅用既有失ACK调用方核对新增组合，之后回到必要语义/择时，不重复已闭合的小链或扩通用crash框架。
...
2026-09-22用户确定3.3先内化Jev、Laya和classifier-dev的适用思路，通过宿主已有模型、支持的工具及必要的Accord组件实现职责；这些专用第三方决策模型/服务的产品接入、封装和用户部署支持留待后续版本评估，不继续占用3.3试用或集成工序。既有工具、插件、服务及成熟开源实现仍按价值复用，用户已选择的组件不被退役。此决定不新增自训模型、托管服务或通用决策引擎项目，也不宣称自有方案已经优于外部模型。

内化按实际问题落地：用足够的小问题和当前必要状态承接语义判断，结果尽量可结构化核对；可直接查询或计算的事实交给宿主接口、代码与测试；未知、置信评分与实际正确性分开，按反馈纠偏、必要时回到足够能力，并检查完整结果及资源后态。组件依情境组合，不形成固定模型顺序、额外每轮调用或关键词SOP。3.3验收继续要求现有条件下的完整结果、用户负担与净影响，17项必要范围和发布门槛不变。

Laya参考固定[42626c3](https://github.com/NandhaKishorM/laya/tree/42626c348753fbb17572a813127df2278a1ec527)：类型化小问题、条件路由及经验校准可借鉴，非生成输出和概率不保证判断正确。公开演示的四个合成英文样本仅是有界初筛，不能作准确率、性能或Accord系统增益结论；原件在私有accord-laya-review-20260922-01。研究结论已足以指导本轮职责分工，不以额外模型试验或微调阻塞开发。

以下为原研究与试验的时间绑定记录，当前接入范围以上述r29决定为准。
...
- r31 / N33-20260909：明确非技术用户的Skill选择负担由获准协调者承担；纠正r30将默认显式策略过度收紧为只能用户亲自选择的解释。允许真实委托与宿主支持路径内代选，保留停用/排除及来源、控制权、实际效果边界。原生加载机制与普通自动协调分开取证，不新增固定调用流程或验收通过。

- r30 / N33-20260909：区分模型/推理支持、用户开放及实际选择；明确第三方Skill盘点、隐式策略、显式使用和必要独立适配的边界，映射既有W02/W06/W08及A02/A06/A08。保留配额中断前原生组合工序及未完状态，不新增固定模型梯子、通用技能路由器或验收通过。

- r29 / N33-20260909：3.3内化专用决策模型的适用设计，借宿主已有模型与工具完成必要职责；Jev/Laya等专用第三方判断引擎接入后置。停止本版相关追加试用/接入待办，保留原研究和试验，不扩大为拒绝一般生态，不新增训练/托管项目；用户部署负担与完整质量底线保持。
