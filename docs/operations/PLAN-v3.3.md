...
后续执行连接采用随包Node适配器，复用宿主已持有的原生控制连接、作用域持久记录与独立核验，不新增常驻服务或私有观察体系。先完成控制者可用条件下的实际派发、多轮接管与失败分支，再接入具备真实入口条件的自主任务；普通Desktop/IDE未提供该连接时保持未知，不搜索私有IPC或另造用户入口绕过边界。工序仍服从实际依赖，原生压缩和先前普通任务/受控交接结果按精确范围复用。
...
私有试验已准备合成样本及事先冻结判据，当前受调用准入限制，尚无可用于采用决策的模型结果。账户、密钥、费用与原始测试记录留在私有试验目录，公开文档不发布未经许可的基准或性能信息。Gemini报告提出的架构、15ms、90%、2KB、统一3秒或“模型确认后释放锁”等描述均未形成当前事实或规格；先查权威来源及适用实现，再按真实条件验证。免费促销不能成为长期零成本承诺；本版也不以新增专用付费模型为前提，不承诺零Token、CPU或时间成本。开发者一次承担试验费不等于开放客户端可分发共享密钥或无限补贴。可选评估器不可用、超时或拒答时回到已有足够能力，必要未知继续保留；不能为退路而放松授权、暂停和完成条件。如后续取得准入则沿原样本继续，先验证任务适用性、总体成本及退出方式，成立后才修改实际组件和受影响验收绑定；外部准入、邀请或试验结果均不阻塞独立开发主线。
