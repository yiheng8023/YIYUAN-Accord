...
节点：N33-20260909 · 修订：r14 · 2026-09-13。状态：保留整体交付目标，明确全局校准及条件性保留/适时退役，功能开发未完成，发布未授权。
...
ChatGPT既有获准侧载与有限兼容性检查保留为历史，结果见[工序](PROCEDURE-v3.3.md#chatgpt入口候选准备2026-09-09)。原完整12文件包保留；派生Skill实验不关闭W02/W06，也不建立普通Chat的独立交付义务。下一Agent切片由真实任务需求触发所需能力，并检查自主择时、实际动作和完整后续；先按入口适用性判断是否继续某条候选路线，不能因导入成功自动扩大测试。
...
| Codex VS Code入口 | 2026-09-12用户更新后，真实界面提供插件管理、YIYUAN Accord市场条目及钩子入口；Code子进程实际使用26.908.40401内置的0.154.0-alpha.6.2 | 按当前构建和账户核对具体包加载、Hook执行及交付闭环；撤回依据概括性文档作出的“不支持插件”判断，不预先排除，也不从界面可见直接宣称验收通过 |
| ChatGPT Work / Codex云端候选 | 官方确认实际执行能力；先评估必要职责、用户收益及共有执行基础的复用 | 绑定具体执行位置、权限与数据/状态边界，判断全链路可行性；依赖Hook的当前包需核脚本、Node和信任，缺失时比较职责等效方案，不据此直接排除入口 |
| 普通Chat辅助候选 | 2026-09-09侧载及虚构三文件任务保留为有限历史；Accord实际参与未证实，不列为默认独立交付项 | 只有明确的对话、需求承接或执行衔接价值才继续；按实际承担的职责验证，不从Skill选择、产物或Work说明推定完整适配 |
...
2026-09-11 正式观察先聚焦两个已定义的 App Server 子范围：生命周期与任务内 Skill 暴露。宿主条件按实际 CLI 0.154.0 校准，沿用原判据、范围及无真实模型的本机协议夹具；在新运行前绑定不可变候选、完整包、原生接口、控制器及观察来源，再独立核验与复查。安装、受信任加载、中断恢复、暂停保全、候选拒绝、卸载和暴露恢复分别取证，不能合并为自主协调或整个宿主通过。旧 CLI 普通交付案例保留原定义，待相关新运行前另行校准，不为更新版本号而重复订单试验。最新准入结果及其精确对象记入接续和试验记录；上述量化基点保留其观察日期，不能作为实时分数或将旧观察升级为新证据。
