...
本批继续复用原生持久历史，已接通已settle目标的新controller恢复；Windows单次真实固定响应、原件保护和独立回读通过，Linux/macOS同批原生CI及artifact独立回读也通过；整体CI的离线测试目录别名错误另行修正，不改原生通过范围或放松判据。正常释放后恢复与未确认效果/异常中断分开，后者先按既有持久回执查后态，不清active transfer或自动重放。已核协议与两轮交接不再重复实验；后续工作针对真实择时、普通入口接入与必要失败恢复的剩余依赖。
