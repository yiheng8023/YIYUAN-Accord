...
节点：N33-20260909 · 修订：r30 · 2026-09-23。状态：3.3内化适用外部设计并借力宿主完成职责，专用第三方决策模型集成后置；功能开发与必要验收未完成，完整验收后发布3.3的条件授权保持。
...
第三方Skill协调并入W02/W06/W08和A02/A06/A08：先复用当前宿主及原管理器提供的盘点，区分安装、启用、元数据可见、隐式选择许可、显式调用和实际执行。源文件与用户组件保持各自所有权；允许隐式选择不保证每轮被选中，明确关闭隐式调用也不等于技能无用。Accord可按语义与任务条件发现并协调适用能力，但不得通过自动读取正文后照做、复制包装或改写用户AGENTS.md绕过当前调用限制。显式请求在宿主支持与任务授权内使用原技能；确有协调缺口时，先核实独立适配的可行接口、调用策略和用户选择，再决定是否实现，不先建通用路由器或批量改第三方源文件。适配不能凭自身提示获得高于宿主的权限；调用正确仍须核验完整结果和退出后态。
