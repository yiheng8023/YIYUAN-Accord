...
ac5dd3e8的Linux/macOS同一原生组合已独立回读通过。下一恢复切片复用原生持久会话和现有SQLite标记，先覆盖已settle且前次执行已静止的controller更换：先独立核对当前权限/暂停和效果，再以已确认回执CAS claim，随后resume并核对原生后态；旧依据不能重复消费。未知效果和活跃transfer继续保留，不新增数据库schema或常驻服务。验证用原已结束场景的只读副本，旧证据不得被恢复流程改写；固定响应只回答恢复接线，模型择时与业务恢复仍分别推进。
