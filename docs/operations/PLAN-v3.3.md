...
自然语言意图以完整输入、上下文、当前状态及授权关系判断，不绑定中文或有限关键词、同义词表；否定、条件、引用与不可信内容不得被孤立命中当成操作授权。原生事件名和内部精确回执可用于协议关联，但不代替语义判断。现有UserPromptSubmit处理有效输入、保存来源并提供协调职责；中文“同意/暂停/继续”的已观测案例只证明对应样本，不能扩张为多语言或任意表达覆盖。按实际适用语言与风险补充同义改写、跨语言及混合表达的行为反例，不穷举词汇或另造触发引擎。
...
普通插件入口先复用宿主MCP调用元数据接到已有状态诊断：inspect_task_state仅读取既有根任务记录，不模拟原生dynamic请求、不派发接管，也不新增存储。由宿主管理的stdio进程按实际启用/加载/退出核验，不承诺零占用；缺失元数据或共享会话与线程身份不一致时保留未知。后续写入、恢复及交接连接按真实缺口分别绑定，不能从这个只读入口推成全链路完成。该工作属于W03/W05/W06/W07已有范围，基线与验收保持原质量底线。

实际采用后的下一连接是按需将Hook绑定的上下文读数与当前MCP调用身份合并，复用已有预算判断；默认读取仍轻量。原生历史头并非当前宿主版本，新鲜度按具体工作绑定且不重标旧数据。开发期间使用宿主支持的独立构建标识使更新可识别，握手版本与包身份保持一致；这些实现细化不新增状态层、通用控制器或固定交接流程。
