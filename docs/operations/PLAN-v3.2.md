...
- `claim-error-isolation` — `source-reviewed-public-regression-open`：独立复核定位 admission.py 的全局 not errors 及 control.py 公共返回的同类合并条件：单个生命周期证据错误会连带使未受影响的功能/价值声明失效。局部计算反例成立，公开验证入口回归尚未运行。需要最小隔离个案与全局主体/来源/评审错误，同时保留所有错误及整版拒绝；此缺陷不解释此前默认追逐 Claude 的排程选择，也不优先于必要结果验证设计。
