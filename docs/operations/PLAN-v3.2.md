...
准入契约：`yiyuan-accord-evidence-admission/v3`；已声明 2 个作用域、2 个用例。数量不是通过记录。
...
- `codex-function` — `codex/cx-sdk`；Current Codex task behavior through direct App Server in the authorized existing-user composition. This scope retains the whole mapped functional and relevant scenario obligations; a narrow passing case cannot close the remaining inventory. Each case binds its actual selected or implicit entry, package, material configuration and observed extensions. This is not pristine-host, Desktop, Chat/Work, SDK-library, complete lifecycle or incremental-value acceptance. Separate cases remain necessary for materially different conditions and missing effects; do not infer full context-pressure or archive-prevention behavior from same-carrier steering. 用例：codex-installed-selected-continuation。
