...
- `source-to-projection-convergence` — `native-activation-characterized-convergence-open`：Both Skills remain unpublished dev.9. Earlier selected management failed despite body loading; the ordinary neutral pair also failed with no candidate body use. A zero-model native probe now confirms that a namespaced command delivers the exact body before inference. Narrow paths plus an actual matching Read did not deliver recognizable body in this inline-plugin condition; do not generalize all host paths or ship it as a fix. The earliest observed management gap is failure to reach a bounded, truthful hold after sufficient bad-source and prior-state evidence, not a located deterministic code defect. Bind one same-candidate ordinary/native-selected neutral comparison to test whether early delivery improves required checks and convergence. Keep permissions, route, budget, body and earlier failures; indirect behavior/cost remains in impact accounting. No gain or whole-product admission is established. Function, guarded update, independent recovery, Desktop continuity and value remain open; shared installation stays deferred.
