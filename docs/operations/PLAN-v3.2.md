...
- `claude-code-lifecycle` — `claude-code/cc-cli`；Use a verified task-owned configuration/cache/profile only; no shared candidate installation before accepted publication. Check live manager state and independent installed bytes. Complement installation state with the same installed candidate loading/activation and selected normal-entry effect under the existing authorized route, bound before a model call. The native manager survives Skill absence and owns each supported operation. Observe failed-update post-state before later operations; evaluator restoration cannot supply native recovery. Restart activation is not hot reload, whole-host functionality or a new account/route. 用例：claude-native-plugin-lifecycle-and-activation。
...
- `source-to-projection-convergence` — `implemented-local-unverified`：Both current worktree Skills are unpublished 3.2.0-dev.7. Guidance separates revisable plans from task authority, budgets continuity early, re-senses after compaction, distinguishes fact checks from confirmed takeover and requires explicit user authorization for task archival. Current reference copies remove the retired prompt and its bindings; exact historical definitions remain in Git. Codex dev.7 installation and a verified 3.1 rollback remain historical diagnostics; the candidate is now uninstalled under the user’s revised publication-first direction. Claude was not changed. The controlled App Server sample delivered 140 then 60, with no observed Skill body read. Whole-duty, Desktop, failure, lifecycle and value acceptance remain open; see codex-installed-dev7-and-recovery.
