...
- `source-to-projection-convergence` — `implemented-local-unverified`：Both current worktree Skills are unpublished 3.2.0-dev.8. The host-Agent now has explicit package preflight, installed/loading checks and surviving recovery responsibility after the native-update safety premise failed. Guidance remains conditional; early context budgeting, compaction re-sensing, confirmed takeover and explicit archive authority persist. This is a local projection change, not verified Agent execution. Historical dev.7 and earlier observations keep their original package/host subjects. Shared Codex candidate installation remains withdrawn; shared Claude stays at 3.1. Current selected composed lifecycle, full-function, Desktop and value acceptance remain open.
