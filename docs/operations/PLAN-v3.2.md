...
- `codex-lifecycle` — `codex/cx-sdk`；Controlled native Codex plugin lifecycle in a disposable profile. The native manager executes each authorized operation; the maintainer orchestrates the experiment and separately observes raw RPC, files, active exposure and process state. One install/update/uninstall intent must own its required transitions without evaluator repair. Fresh-session exposure is distinct from Skill use; a cache or catalogue alone cannot prove loaded state. The initial invalid-update case covers rejection before writing and preservation of the last safe state, not rollback after partial mutation. Uncovered failure windows and ordinary-entry activation remain open until observed. This scope cannot establish Desktop continuity, model behavior, whole-product functionality or incremental value. 用例：codex-native-plugin-lifecycle。
