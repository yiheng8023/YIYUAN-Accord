...
- `source-to-projection-convergence` — `ordinary-entry-effect-gap-and-limited-impact-counterevidence`：Both current Skills remain unpublished 3.2.0-dev.9. Earlier selected management attempts failed despite body loading. A new ordinary-input neutral-target pair also failed: native without Accord reached the turn limit; candidate with catalogue and empty startup Hook exposure timed out without Skill/body use. Preserve actual authority failures, denied actions, task-local residue and shared invariance. The native failure shows Accord presence is not necessary for this sample failure; it does not exclude added or indirect impact. Path rewriting, permission interaction and differing termination are observed behavior/cost, not proof of an independent external cause. No gain is established. Inspect the ordinary-entry trigger/caller before a minimal repair; do not blindly change unread guidance, force every task through a Skill, expand permission or increase budget. Full function, guarded update, independent recovery, Desktop continuity and value remain open; shared installation stays deferred.
