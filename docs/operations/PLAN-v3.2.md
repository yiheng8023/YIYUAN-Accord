...
- `sequential-correction-observer` — `implemented-offline-verified-real-observation-pending`：按用户确认的两轮真实消息顺序、实际文件变化、任务边界、结束/清理边界复验。两个二轮缺失新写入/回读证据的反例先失败，再通过非末轮result失效旧轮记录修复；保留单轮checkpoint和最后一轮回读。96项源/准入/观察器回归通过后，独立Spec复核实跑发现目录内Read探查被误判越界；该反例也先失败，再按解析路径是否处于工作区修复，18项观察器回归通过。保留目录外读取、输入写入、禁用工具和未闭合进程的拒绝，离线任务资源已清理。先同步完整实现与判据并提交A，再收集同一活进程140→60的实际观察E；这些维护诊断不补齐整项职责、不证明自主恢复或增量价值，真实scopes/cases及独立来源准入仍未完成。
