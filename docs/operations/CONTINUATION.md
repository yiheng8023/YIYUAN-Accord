...
当前导航，更新于 2026-09-09。3.3 已获授权进入功能开发，首阶段仅聚焦 **ChatGPT/Codex**，跑通后再考虑 Claude 和其它宿主。当前四文档为 r2；3.3 产品代码尚未实施、功能尚未验收。以下当前导航优先于后文保留的历史准备、修复与发布状态。

## 当前目标与唯一入口

- 用户带着想法来，负责关键决策、必要授权、验收和主体责任；宿主、生态与 Accord 承担其它已授权工作。功能实现优先，主动发现能力、推进、纠偏、恢复、连续性与收尾，减少用户催促和救场。
- 当前文件：[基线](BASELINE-v3.3.md)、[计划](PLAN-v3.3.md)、[工序](PROCEDURE-v3.3.md)、[验收](ACCEPTANCE-v3.3.md)，保持 F01–F08 / W01–W08 / S0–S5 / A01–A08 映射并随证据修订。原传播目录里的两份 3.3 草案仅作历史提案。
- AI 宿主负责理解和判断；Accord 接通必要任务入口、原生执行/状态/唤醒/反馈及恢复。不能依赖偶尔选中 Skill 才履行必要职责；不要求强制代理所有工具，不另建重复宿主控制平台。需要运行时或持久状态就补必要连接。
- Agent 主动查明实际宿主功能、能力、按钮、设置、配置和生态；读取顺序不等于指令主权或执行优先级。尊重用户自定义环境，和宿主/生态共生互补、各自主权独立。ChatGPT 与 Codex 的不同入口不得互相代验。
- 最新补充：3.3 承接可靠历史功能与机制，不推翻重建。信息需要核实、成熟方案或更佳效果有实质可能时，先主动联网及查找 GitHub 等可信实现，比较后行动；原生能完成不等于路线最合适。生态能力要覆盖启用、停用、复用、回收和退役，避免无限增加常驻负担。
- 资源管理必须触及操作系统与宿主运行态：CPU/GPU、内存/显存、磁盘容量与 I/O、进程、终端、浏览器、服务、并发任务与子代理的需求、分配、压力响应及回收。需主动调整并发或释放归属清楚的任务资源，核验负载和后续工作；临时文件清理不能代验。不得盲杀共享/未知进程或退出安全防护。现有 `product/reshaping-guidance.json` 的 `resourceStewardship` 已有职责定义，需承接并核实实际闭环；源码关键词初查没有找到完整设备负载执行器的证据，不能将此有限检索当全部能力不存在的证明。借宿主/系统完成同样有效，不要求自建操作系统调度器。
- 已授权新任务继续开发及按需排除变量；优先任务级、可回退的隔离与局部调整，并回到真实自定义环境验证。共享改动查明实际范围、影响任务、备份及恢复；不将此理解为升级宿主、任意安装、变更其它项目、取得新账号/数据或无限费用授权。合理自动化成本不构成必要功能的否决条件。
- `product/development.json` 保留 3.2.1 候选与历史验收范围；其旧“3.3 仅纸面”语句不覆盖上述新授权。开始相关实现时同步整理当前开发定义和受影响检查，不改写旧证据/标签，不以旧 PASS 认证 3.3。3.3 发布须另行获得相应授权。

## 交接与下一实际动作

- 用户明确要求新任务接续；使用当前 `C:/Projects/YIYUAN-Accord`，不复制历史、不新建分支或工作树。来源为“主线程20”，任务 ID `01a07984-457e-75a1-ba39-4e5fda61f440`。当前导航与四文档是交接载荷，具体提交以创建消息和接收端 fresh Git 核验为准。
- 来源先完成提交/推送；接收端先只读独立核对仓库、目标、权限和未完成项。双方明确转移唯一写入职责后，接收端开始实际工作；来源不并行写入。收件不等于接管，本次是用户要求的交接，不能宣称证明自主触发。旧开发任务保留，不自动归档。
- 接收端首先从 ChatGPT/Codex 普通入口的可靠职责接入与实际推进链切入，检查当前源/目标宿主可用接口，定位一个实际断点并实现验证；首个切片按计划可调整，不能把新一轮全量资料整理变成前置。必要时对子任务使用并行代理。
- 历史 前身系统 的持久计数 Hook 已找到源码线索（基线含精确链接）；尚未定位用户实际成功体验对应版本与链路。调查与当前修复可并行，不整代回滚，不用“缺少历史统计”拒绝当前功能修复。
- 现安装 3.2.1 曾逐字节核验，当前主任务收到输入 Hook；这些是已有范围事实，不证明新任务、全部入口或净收益。先只读重核相关环境，再安排必要受控变量与真实环境验证。
- 用户授权及时清理不再复用的项目测试会话（含 Codex/Claude 历史、当前、后续），先辨明归属并保留必要证据；不包括开发主任务或未知用户会话。

## 传播撤回与资产

- 用户于 2026-09-09 要求先撤下 3.2.1 宣传视频；已通过实际平台界面完成并回读核验：B 站 `BV1haYJ6ZE5H` 仅自己可见，YouTube `eC7IQGTp8I8` 私享；X 中英文视频帖 `2097336959643582802`、`2097335402365669385` 已删除，原链接均显示不存在。核验时点为 2026-09-08 16:24:43 UTC。GitHub 产品 Release 未被要求撤回。
- 素材和当前撤回记录属于私有 `C:/Projects/YIYUAN-NEXUS/communications/campaigns/accord-3.2/`；保留中英文原片、字幕、音乐、字体与许可、文案和发布历史。品牌目录支持多个项目，产品仓库不承载传播资产。原下载副本已按字节保全后清理，不重复制作或重新发布。
- 品牌仓库撤回记录已提交并推送至 `86e5f3a`；原先发布链接与状态作为历史保留，当前 JSON 状态为 `promotional-videos-withdrawn`。本次只撤回视频，X 的文字作者/版权回复未改。
- YouTube 高级功能审核及版权检查此前待结果，用户要求等待；当前已下架宣传，不需要以补外链或宣传为由阻塞开发或重新发帖。重新传播须重新核对届时实际产品与用户发布决定。
...
- 传播曾完成并归档到品牌仓库；现已按上述最新要求撤回，当前转为 3.3 功能开发，产品实际使用边界继续保留，不把传播完成当作使命验收。
