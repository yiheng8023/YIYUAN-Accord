...
This file is current navigation, not product authority. Recover live Git,
remote, host and product truth before continuing. Historical release procedures
remain in Git and `docs/operations/HISTORY.md`; they do not become an active
route by being readable.
...
2. `product/reshaping-guidance.json`
...
- Historical Commit A `7ea91dad811d337f00f75eb521cffacabd73b05f` froze one
  ...
  and arbitrary orphan-cache files could satisfy cleanup. The historical schema-v4
  ...
  inherited. Frozen revision `fc9c1a7a64257ddf315f862a091a081c4104d81b`
...
- Exact `fc9c1a7a64257ddf315f862a091a081c4104d81b` completed the schema-v4
  replay and published the 54-command public record with SHA-256
  `f9170d433cde999c4aaeb4e013eb79663cbe6cb97935f9cc94ac41e382e32dce`;
  the tracked LF-normalized repository carriage has SHA-256
  `88c275c7c4e9b14a624f9575b3dc3a4b295e89a059b6d969d44d2c0d606fb647`.
  Their parsed JSON is identical, but their byte identities are not interchangeable.
  The then-current node, lifecycle and transition validators returned no
  errors. Later review found two evaluator defects outside those predicates:
  global system-process command-line enumeration breached the intended privacy
...
- A 2026-09-01 current-host check found three distinct states: the Codex CLI
  ...
  the already-open task exposed the 3.0.1 Accord Skill. Claude Code also retained
  enabled 3.0.1 at immutable ref v3.0.1. This is an Accord-enabled composed
  development lane, not a clean baseline, and directly disproves treating file
  refresh or list output as hot replacement of already-loaded behavior.
...
- At the tagged v3.0.1 SHA, the released program remains historical `ready`
  ...
  counterevidence. Fresh isolated GT-12 at `72709b8` now passes only zero new
  task-worker admission, exact exclusive release, four shared-resource checks,
  evaluator-owned lease completion and cleanup; one blocked read-only command
  remains a bounded route correction. GT-13 through GT-16 plus GT-18 and GT-19
  initially had changed Skill, evaluation contract or reference-core subjects.
...
- GT-07 (`carrier-fitness-with-unknown-capacity`), GT-11
  ...
  remain exact historical counterevidence; they are not rewritten to match v3.
  The historical Claude `GT-07:cleanup` failure remains counterevidence and
  continues to exclude current Claude and cross-host cleanup equivalence. The
  current release claim therefore retains `GT-07:claude-code:cleanup` as a scoped cross-host
...
- Codex Cloud remains a condition-fit hosted evidence lane, not a permanent
  ...
  condition-limited Cloud failure remains a recorded scoped limitation, not a
  permanent product or future-host failure and not failure of required local or
  GitHub-hosted conformance.
...
- Resource stewardship is now a first-class host-neutral dynamic loop:
  observe, attribute, admit, monitor, rebalance or degrade, release task-owned
  exposure, then verify. Native whole-loop governance is a passing no-op and
...
- the user supplies the outcome, consequential judgment and unavoidable
  authority;
...
residue changes. Visibility and installation do not imply activation.

Do not add a graph database until representative route or impact queries prove
that JSON plus an in-memory projection is insufficient for scale, query
...
14. `reaccept-whole-system-after-live-state-closure` — active. Prior source-bound
    ...
    rejected that contradiction and passed the production verifier, focused and
    current-contract tests, alignment and all four pre-candidate reviews. Its following readiness probe
...
The correction starts at the product premise and constitution boundary.
Recompute or replay:
...
them to simulate current evidence.

## Boundaries

- Work only in the canonical checkout unless a later task explicitly changes
  that boundary. Do not create a branch, worktree or repository fork for this
  continuation.
- Do not use Codex Security, TAC, trusted-access review or credential/session
  logs for this work.
- Do not install, enable, connect, spend, publish, deploy, destructively clean
  or cross a new trust or data boundary without the authority required for that
  effect.
