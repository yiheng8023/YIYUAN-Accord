...
更新：2026-09-21 · N33-20260909 / r28。b5bc28fb完整CI已11/11通过；本批补SDK源任务调用者与SQLite持久写者记录。资源/环境原案报告遗漏仍不准入，指导修正仍待必要行为核验。以实时Git、工具目录和当前receipt为准。
...
CI节奏共识已按9月20日用户确认写入计划：本地小改/针对性核验与提交可以及时进行，相关变更形成完整工作段后再集中推送，避免尚未完成的有效全矩阵反复被取消。必要平台覆盖和最终候选门槛不变；纯接续记录不触发无价值重跑。当前b5bc28fb完整CI已11/11通过，此后是否推送仍按工作段及实际验证需要判断，不回到每个对话小片立即推送。

## 当前事实

| 项目 | 已核事实与边界 |
|---|---|
| 仓库与写者 | 本批起点main与origin/main均为b5bc28fb、工作区干净。root负责连接/契约/文档集成；两个Sol子代理已交付并停止写入，交叉复核发现的异常回执重复执行、写者变更停写及非法JSON答复问题已修复。相关实现、修复和记录集中推送，不按每轮小改触发矩阵。 |
...
| 当前源候选 | 3.3.0-dev.1+codex.20260921082457，24文件，SHA 125b5bd01078e7486ea5b41489ce8557d759211ba6fb326040e5b8554d7e7a2c。新增SDK source session、可选SQLite recorder及原子事件接收；共享安装仍33628，当前源行为与安装行为分开。资源/环境报告指导修正仍待必要行为核验，原失败不改标。 |
...
| 已完成CI | 精确b5bc28fb的[35569973358](https://github.com/yiheng8023/YIYUAN-Accord/actions/runs/35569973358)已11/11成功；f3cf8951的[35560235516](https://github.com/yiheng8023/YIYUAN-Accord/actions/runs/35560235516)已11/11成功，两平台MCP原件另行独立回读通过；前批7e08773c也11/11成功。前批Windows/macOS完整回归约38–43分钟；不因正常等待另触发矩阵。下一批精确状态按GitHub回查。 |
...
| 当前SDK机制检查 | 真实connection + carrier core + SQLite组合通过；连接26、记录器7、session8、既有handoff29，共70项本地通过；开发契约87/87（148.531秒）通过。三项静态契约检查通过；实际代码/测试2437451字节、169文件、主指导13785字节。未调用真实模型或安装新包，不是A05整项/GUI准入。 |
...
1. **完成SDK真实调用接线及有界验证。** 集成源创建/工具注册/事件循环与现有handoff核心、真实SQLite；覆盖畸形回执不得重发、写者变更不得继续源写入、并发CAS和故障重开。固定协议结果只支持机制，不作为普通模型择时或全A05通过。组件借用同一owner控制连接；其它组件请求按归属处理，线程级/全局权限不能混淆。未知效果和目标重复交接继续按原缺口推进。
...
fresh核心已有carrier-handoff.cjs、runHandoffProposal和stdio/WebSocket连接；原完整调用者及SQLite ledger此前只在测试中组装。本批新增随包codex-session.cjs创建源任务、注册两动态工具并泵事件，carrier-recorder.cjs用Node内置SQLite保存scope/revision/lease；无需集成方复制测试实现。仍要求实际获准且幸存的已初始化连接、当前权限/计划与独立语义核验，借用资源由原owner收尾；失败不重放、转移后源不可续写，目标重复自动交接与断线后的controller恢复尚未闭合。MCP状态工具不替代App Server控制，不新增常驻服务或私有IPC。官方loaded-thread resume可重放pending requests；另一个订阅者仍在时源连接关闭不等于释放。dynamicTools需thread/start注册，resume不补注册；排队用户消息也不是只读探测。
