...
主代理在独立reviewed-deliverables路径纠正四份可用草案：恢复Agent核验责任，清单改为首次使用者，核对既有发布授权及条件，并去掉临时目录措辞。独立内容复核无剩余实质缺陷，已并入现有LAUNCH-v3.3.md；这是评估者校正后的W08材料，不是原02自主通过。原案的observe/recheck只可提交真实失败/未知事实，不伪造通过的四轴审查包；A04仍须原system-integration，其它必要范围不减少。两轮失误没有证明上游注入或控制连接失效，不再盲目补协议、修改关键词规则或从单例推断某模型全面优劣。下一实际工作先按这些事实调整必要语义核验和执行分配，复用本次有效材料；不得原样重放本例填计数。原件、资源核对和纠偏记录在accord-release-copy-20260924-01。
