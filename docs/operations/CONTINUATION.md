...
| 两宿主安装、更新与独立恢复 | 进行中 | 安装后生效、实际更新、坏源保护、损坏后的幸存接管者恢复及退出。 |
...
- 较早 `claude-ready-orders-correction-dev6`、Codex capability-loss/conflict 及
  ...
  不为沿用旧入口重复完整测量。
- `claude-lifecycle-target-identity-rebinding`：复查确认现有 manager/helper 可执行，
  dev.11 检查的是旧 3.1 且未带目标摘要，刷新后因轮次上限未继续。已按冻结文件
  清单补齐可信候选身份，重绑 dev.15 方法，推进来源重核、实际更新与独立恢复；
  旧失败保留，新生命周期尚未执行。三 profile、指定 Skill/helper 和激活数值是
  可调整手段，本次隔离按缓存污染前提选择；不新增执行框架或共享安装。
