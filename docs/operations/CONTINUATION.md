...
更新：2026-09-21 · N33-20260909 / r28。f3cf8951完整CI已11/11通过，共享安装已更新；本批前瞻绑定资源/环境联合场景，尚未执行真实模型。以实时Git、工具目录和当前receipt为准。
...
| 当前源候选 | 3.3.0-dev.1+codex.20260921033628，22文件，SHA 8f749addccd25d111ebb6feaab7287009d2072dcaad109facdd48ff9ffe75213。源与共享安装均含manage_task_state；当前Desktop写入工具采用仍未知。本批只新增验收定义/夹具，包字节不变。 |
...
| 当前场景检查 | 当前准入类19项中1项因新增范围后旧预期未同步而失败，原结果保留；修正后受影响2项通过。新案例与源码仍需最终静态核验，尚未执行真实模型。 |
...
1. **执行一次资源/环境联合场景。** 先提交前瞻定义、两份复用夹具及判据，冻结精确Git和实际安装身份；再通过正常SDK/AppServer发送唯一普通输入，模型Terra/medium已从当前原生目录确认可用。使用现有Windows sandbox，不开启Plan/Goal、不注入source Hook、不预批准写工具、不预绑checkpoint、不发救场提示。先核当前包参与与Goal/mode，后独立核容量变化、缓存复用、八份结果和资源/共享配置后态。真实模型尚未运行，不能把19项准入回归或机械预检当通过。
...
本次只读审查发现观察器可能在配置恢复失败时仍以0退出；已在真实模型执行前修正，并补当前Hook指导/原生mode/Goal/最终用量与资源后态核对。输入生成器从本次工具调用原文执行后留存，未冒称事前冻结；真实模型准备时与压缩原件一起冻结。机械预检属于较早工具身份，最终显式runtime-root、CPU查询和cache前后快照增量只按其实际检查归因，不重盖预检。
...
| accord-resource-change-20260921-01 | 复用既有批处理/WindowsJob的新容量变化场景。fixture-src保留唯一机械预检与最终源身份；observer/recovery源码已准备并审查，当前尚未启动普通模型。准备后的binding、raw native、独立成果与资源核验分别留存，不能提前填PASS。 |
