...
**当前待接续：IDE用户说明审查已准备、未派发**。accord-ide-copy-review-20260926-01冻结9afdc706的中英README、LAUNCH、BASELINE，以及有来源的事实快照/清单，共六份原件；两份允许产物为review.md和claims.json。此为W08必要的用户说明一致性审查，同时观察现装202615在真实IDE的参与，不重放旧readiness任务或预填验收。独立检查无实质阻断，已区分消息原始字节与LF文本Hash，原消息未改。现装24文件与冻结Git匹配；VS Code已打开独立workspace，共享配置Hash未变，尚无业务输出。Computer Use的guidance.md明确禁止自动操作Windows应用内的Codex扩展，所以需要用户在IDE发送已准备的message.txt；主任务负责随后核对原生输入/任务/条件、指导参与、两份产物、原件和资源后态。保持现有模型/模式，不自动配置或开启其它能力。用户发送前不得宣称已执行；材料与窗口保留供此未完工作，不能提前清理或重复派发。
