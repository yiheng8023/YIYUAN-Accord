...
| 根任务状态 | 最新用户输入已自然捕获，当前epoch=20baf501-d132-4279-80cc-294a2a078128，原生MCP读回一致；此前retained-native-replay恢复保持历史身份。无需继续恢复或调查压缩。独立0.157.0原生只读查询确认当前持久任务身份与goal=null，仅说明读时点；任务创建端记录仍为Desktop0.155.0-alpha.16.4，不混为同一宿主版本。 |
...
**普通CLI的Goal原生读回接线**：持久运行可在完成轮次后只读同一任务的thread/read与thread/goal/get；准入绑定运行自动采样，其它运行按需启用。绑定持久rollout、线程、工作区、原始RPC及读取时点，未知不补false；复用所属进程控制。已修原始响应内容未核对、POSIX释放误判及附加读回中断丢失已完成阶段三项审查问题，并绑定全部读取依赖源码。92项入口回归、Python3.10两项关键反例及verify/host-check通过。当前既有任务的真实API读回已证接口可用；新普通CLI交付链、成品语义和准入仍待下一真实审查任务，不能由离线检查或单次Goal为空代验全程行为。
...
1. **普通调用者采用与组合**是主要缺口。SDK接口和固定协议已实现，本批普通source恢复的三平台验证已闭合，不继续复制相同夹具。普通插件主线应检验宿主Agent、入口指导、必要状态与真实工作结果的组合；新增App Server调用者或启动器不是所有入口的共同前提，只有具体fresh路径确需时再判断。为下一项确有必要的真实工作事先绑定主代理、worker、内部核验与独立评估者、实际包/条件及observe/recheck；发现调用断点再做最小接线，不制造业务任务填计数。
   接管后的独立源码核对确认，现有bind_evidence_execution、调用方observe/recheck和独立reviewBundle已有连接入口，尚未发现须另建通用调用器的依据。80ad291e的托管修复已闭合，不重复该夹具；115917已原生安装并独立回读，新输入自然捕获也已确认。当前补齐普通持久CLI的完成轮次后Goal原生读回，先修正源码审查发现的回执配对、跨平台退出判据和中断前原阶段保留，再用本次必要的独立代码审查成品验证实际普通调用链；不制造业务任务或重复旧失败。
