...
A research or proof item may enter active work only when the current causal
increment names:

1. the product criterion it can change;
2. the observed uncertainty or failure;
3. why current evidence is insufficient;
4. the smallest source and execution boundary;
...
v1.2 is ready, and O1-O5 are 1/5. Two bounded outcome-neutral slices committed
an O4-specific validator and bound its public-safe projection directly to raw
Codex host and repository observations before later registration, then exited
the graph. Neither performs O4 measurement or earns outcome credit. Its
...
runtimes are living leads for the same reason. Their documented surfaces reject
...
Keep listing, acquisition, installation, enablement, exposure, invocation,
instruction delivery, behavior, value, portability, production, acceptance,
release, and publication distinct. Evidence at one state cannot promote the
next state without its own observation and authority.
