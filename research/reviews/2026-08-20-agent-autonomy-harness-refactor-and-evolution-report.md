# Agent Autonomy Harness 重构与长期演化报告
...
当前 Agent Autonomy Harness 已经经历了多轮设计、实现、实验、失败、反证和修正。这个过程没有白费，它帮助项目逐渐从最初偏“工具与 Skills 管理”的思路，收敛到了一个更本质的问题：
...
> **如果今天基于已有全部认知重新设计 Agent Autonomy Harness，最少到底需要保留什么？**
...
就把它们全部变成 Harness 产品边界。

真正判断标准应是：

> **如果 Harness 不承担这件事，是否真的存在没人负责、并且会影响用户结果的协作缺口？**
...
> **Harness 不需要保证任何任务最终一定成功，但必须保证关键失败不会失控。**

这是对“兜底”非常重要的校准。

兜底不能理解成：

> 原生失败以后 Harness 必须拥有完整替代实现。
...
这可以显著避免 Harness 最终变成一个巨大的工具平台。

---

# 四、第一项重大校准：必须系统吸收上游官方能力和推荐实践

下一代 Harness 不应该继续闭门推导宿主应该如何工作。
...
> **很多 Harness 想解决的问题，应该先问宿主或生态是否已经存在成熟机制。**
...
Residual gap
↓
Minimum Harness supplementation
```
...
因此 Harness authority 不应无限扩大 operation table。
...
> **任何 Harness-specific residual mechanism，在诞生时就必须说明它何时失去存在价值。**
...
因此 Harness 应加入一个适用于强 Agent 的元规则：
...
> 当宿主原生能力已经能够安全、可靠、低负担地完成任务时，Harness 不增加无必要的流程、工具、审批、状态、拓扑或持久资源。
...
### H10 — Native Improvement Retires Harness Logic
...
| 当前 Harness 机制 | 宿主原生能力 | 官方推荐 | 实测行为 | Residual Gap | 决策 |
|---|---|---|---|---|---|
| A | 已解决 | 推荐原生 | 有效 | 无 | 删除 |
...
> **Agent Autonomy Harness 是一套轻量、Agent-neutral、以用户目标为入口的人机协作契约。它以真实需求、真实环境和真实结果为校准，遵循减法、克制、兜底和补位原则：优先复用宿主及现有生态已经充分的能力，不重复制造已有机制；不因为能够控制就扩张产品边界；只针对真实存在且应该由 Harness 承担的残余责任缺口进行最小补位；并在宿主失败、能力不足、环境不可观测或执行发生偏差时提供必要恢复、降级和诚实停止能力。人始终保留对目标、重要专业判断、信任、数据、成本、不可逆影响和最终责任的权威。具体模型、工具、Skill、插件、MCP、Agent、Git 拓扑、任务拓扑、上下文机制和宿主实现都是可替换、可演化、可退役的实现层，而不是产品本体。**

同时增加正式负定义：

> **Harness 的价值不以自身介入次数、控制范围、规则数量、代码规模、验证数量或存在感衡量。原生能力已经充分时，正确行为就是不增加 Harness 机制。**
...
> **Harness 应在需要的时候出现，在不需要的时候消失。**

而从长期演化角度看，还有一句应当成为项目持续自检的问题：

> **如果宿主、模型和生态不断变强，Agent Autonomy Harness 有没有能力跟着变得越来越小？**
