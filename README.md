...
**Current status:** v1.2 has completed `increment.v12-o1-lifecycle-suite` and has
one active, outcome-neutral O2 public-observation replay increment after the
strictly earlier seam commit. Its exact prior revision immutably registered
...
The tree contains a strictly pre-registration O2 seam; the active slice may
only add bounded replay of content-addressed, public-safe projections from
Codex JSONL, plugin state and filesystem manifests. It cannot install or
execute the candidate and cannot
...
introduced by the freeze commit is rejected. The exact signing grant, prebound
public-safe signer, binding-only signed revision, and later code pin now validate.
This freeze carries no cohort activation, outcome credit, installation, or
release authority.
