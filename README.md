...
The development package also exposes read-only MCP tools for inspecting saved root-task state, assessing a bounded work span with sourced context forecasts, and retrieving captured input during recovery. They reuse the checkpoint runtime in one host-managed Node stdio process, with no new account or remote service. Captured input is not complete history or new authorization. Host call identity and the selected workspace remain distinct from permission and current intent; descendant/shared-session state is not automatically adopted. These tools do not replay input, resume pauses or dispatch handoffs.
