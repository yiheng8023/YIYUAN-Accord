...
The development package also exposes a read-only MCP tool for inspecting saved root-task state. It reuses the checkpoint runtime and runs as a host-managed Node stdio process, with no new account or remote service. Host call identity and the selected workspace remain distinct from permission and current intent; descendant/shared-session state is not automatically adopted. This entry does not dispatch handoffs.
