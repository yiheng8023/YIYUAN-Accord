...
def delivery_adapter_contract(adapter_id, package_id, *, development_schema=None, startup_entry=False, carrier_handoff=False, native_state_mcp=False, carrier_session=False):
    ...
    contract = {
        ...
        "optionalTaskCheckpoint": {
            ...
            "recovery": "input-failure-watermarks-outside-lock; current-native-replay-with-derived-epoch; unpersistable-loss-remains-unknown",
            "state": "input-hooks-maintain-receipts-even-without-binding; checkpoints-and-failure-watermarks",
            "cleanup": "unbound-reconciled-receipts-on-session-end-or-matched-caller; preserve-unfinished-checkpoints; watermarks-until-owning-state-directory-retirement",
    ...
    if adapter_id == "codex" and development_schema == V5_SCHEMA:
        if native_state_mcp:
            ...
            contract["nativeTaskStateMcp"] = {
                ...
                "stateTool": "manage_task_state",
                "stateWrite": "root-native-metadata-bound bind/pause/retire; caller epoch/revision retained; native turn checked at read and publication; reasons are not authorization; no business-file write or host-mode change",
                ...
                "effect": "read-or-manage-existing-task-checkpoint; optional-context-assessment-is-read-only",
                "identity": "Codex callId and x-codex-turn-metadata; root-thread-only-checkpoint-access; descendant-shared-session-remains-unavailable; metadata-is-not-authentication",
                ...
                "assessment": "optional contextAssessment reuses assess-context with caller-bound epoch/revision/generation, original times and sourced forecasts; fresh native counters; existing transient locks only; no persisted-state mutation or dispatch",
                "lifecycle": "host-start-reload-and-close; releases-plugin-cache-cwd-before-serving; pins-available-context-reader-code-at-runtime-load; process-exits-on-stdin-end; no-listener-or-daemon",
                "limits": "no-identity-storage-or-arbitrary-operation-override; no-input-replay-or-lock-recovery-tool; no-authority-or-whole-completion-claim; no-app-server-control-or-handoff-dispatch",
            }
        if carrier_handoff:
            contract["optionalCarrierHandoff"] = {
                ...
                "continuationReconciliation": "reconcileContinuation correlates a bound unknown first-continuation receipt, current native read and independent verification, then uses the existing recorder CAS/readback; retains original failure and writer; repeated matching records are snapshots only; no new turn, source release or execution permission",
                "reconciledFinalization": "finalizeReconciledHandoff requires an acknowledged reconciled revision/lease/receipt digest and fresh independent authority, pause, effect and writer checks; no repeated continuation or source resume; same-connection unsubscribe or independently evidenced prior-controller closure; exact CAS/readback before existing settle/restore; uncertain effects are not replayed",
                ...
                "ownedConnection": "runtime/codex-connection.cjs accepts caller-owned Node stdio streams or an already-open standard WebSocket; bounded RPC, exact request-anchor replay and live events, native context from the same journal; no spawn, authentication, durable recorder, semantic verifier or automatic timing",
            ...
            if carrier_session:
                contract["optionalCarrierHandoff"]["sourceSession"] = {
                    ...
                    "controllerRestoration": "settled-transfer-or-owner-reconciled-known-source; explicit-acknowledged-inactive-scope-basis; source-origin-binding-and-native-tool-evidence; independently-quiesced-prior-controller-and-reconciled-effects; claim-and-readback-before-native-resume; summary-reads-and-excludeTurns; verified-current-settings-history-and-native-tools; final-token-readback; old-basis-and-stale-executor-rejected",
                    "limits": "caller-supplies-current-authority-plan-and-independent-verifier; uncertain-effects-require-reconciliation-not-retry; no-active-transfer-or-arbitrary-crash-replay; no-arbitrary-thread-adoption; no-ordinary-GUI-control-or-whole-A05-acceptance",
                }
                contract["optionalCarrierHandoff"]["durableRecorder"] = {
                    ...
                    "controllerClaim": "claimScope-CAS-matches-an-acknowledged-inactive-scope-and-rotates-only-its-token; preserves-writer-and-history; no-timeout-takeover-or-active-transfer-clear",
