...
function hint(event, where, currentInput, prior = null) {
  ...
  return {hookSpecificOutput: {hookEventName: event.hook_event_name, additionalContext:
    ...
    'When a checker\'s validity is disputed, use the packaged Skill\'s independent verification guidance before closing the task. For other consequential verification gaps, use its bounded review guidance when needed. ' +
