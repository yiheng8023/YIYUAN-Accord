...
function hint(event, where, currentInput, prior = null) {
  ...
  return {hookSpecificOutput: {hookEventName: event.hook_event_name, additionalContext:
    ...
    'Answer side questions and incorporate corrections while preserving the unfinished goal. Reconcile affected current plans and dependent artifacts with revised decisions and verified results; mark superseded guidance as historical. ' +
    ...
    'Before ending, check consequential output claims against inspected source facts, not just shape or keyword checks. When creating or repairing a checker, test valid supported source states and their dependent outputs, plus plausible false positives that retain expected words but contradict required facts or permissions; match its PASS claim to demonstrated coverage. Missing operational facts remain unresolved; do not invent them or mark dependent work complete. Reconcile remaining work, continuity needs and owned resources; verify attributable cleanup. ' +
