#!/usr/bin/env python3
"""Validate the internal-communications zero-model effect protocol."""
...
def validate_protocol(root: Path = ROOT) -> dict[str, Any]:
    ...
    _require(
        ...
        "Internal-communications evaluator outcome drifted",
    )
    _require(
        ...
        "Internal-communications calibration coverage drifted",
    )
    _require(
        report.get("accessCommsOracleReused") is True
        and report.get("fullScenarioFixtureCreated") is False,
        "Internal-communications evidence reuse boundary drifted",
    )
    _require(
