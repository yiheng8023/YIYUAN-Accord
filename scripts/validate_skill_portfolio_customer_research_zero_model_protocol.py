#!/usr/bin/env python3
"""Validate the customer-research zero-model effect protocol."""
...
def validate_protocol(root: Path = ROOT) -> dict[str, Any]:
    ...
    _require(
        ...
        "Customer-research evaluator outcome drifted",
    )
    _require(
        report.get("candidateCount") == 1
        ...
        "Customer-research calibration coverage drifted",
    )
    _require(
        report.get("researchOracleReused") is True
        and report.get("discoveryConsentBoundaryReused") is True
        and report.get("fullScenarioFixtureCreated") is False,
        "Customer-research evidence reuse boundary drifted",
    )
    _require(
